...
;
	...
	public $views = [
		...
		'emailActivation' => 'App\Views\email\activation',
	];
    ...
    public function __construct()
    {
		if(!file_exists(ROOTPATH . '/pending_install')){
			parent::__construct();
			$this->ownerGroup = service('settings')->read('auth.ownerGroup');
			$this->defaultUserGroup = service('settings')->read('auth.defaultUserGroup');
			$this->allowRegistration = service('settings')->read('auth.allowRegistration');
			$this->allowRemembering = service('settings')->read('auth.allowRemembering');
			$this->minimumPasswordLength = service('settings')->read('auth.minimumPasswordLength');
			$this->userActivators = [
				'Myth\Auth\Authentication\Activators\EmailActivator' => [
					'fromEmail' => service('settings')->read('email.fromMail'),
					'fromName' => service('settings')->read('email.fromName'),
				]
			];
			$this->userResetters = [
				'Myth\Auth\Authentication\Resetters\EmailResetter' => [
